...
\begin{flushleft}
\textbf{Изготвили:}\\
Иван Бърдуков -- фак. № \textit{да се попълни}\\
Мирослав Котларов -- фак. № \textit{да се попълни}\\
Асен Пантелеев Попов -- фак. № 121223246\\
Група 44\\
III курс, КСИ\\
...
Проектът не представлява само графичен интерфейс, а цялостна система с няколко ясно разделени слоя. Мобилното приложение е изградено с React Native и Expo, сървърната услуга е реализирана с FastAPI, данните се съхраняват в PostgreSQL чрез SQLAlchemy ORM и Alembic миграции, а ML компонентът използва XGBoost модел, обучен върху исторически набор от Formula 1 данни. Тази структура позволява приложението да бъде разглеждано като реалистичен пример за клиент-сървърна мобилна система с машинно обучение.
...
Мобилното приложение е изградено с Expo и React Native, което позволява използване на една кодова база за iOS, Android и web режим. В рамките на разработката приложението е тествано през Expo Go на iPhone. Интерфейсът е многоезичен и поддържа английски и български език чрез собствен \code{LanguageProvider}.
...
Схемата се управлява с Alembic. Това позволява промените в базата да бъдат версионирани, възпроизводими и проследими. В проекта има миграции за началната схема за данни, потребителите, сесиите, известията и последващо почистване на отпаднали функционалности за удостоверяване извън финалния обхват.

\section{Сървърна реализация}

\subsection{Технологии и структура}

Сървърният слой е реализиран с FastAPI. Използването на Pydantic позволява валидиране на входните и изходните данни, а SQLAlchemy ORM дефинира релационния модел на данните. Alembic се използва за управление на миграции, а \code{argon2-cffi} осигурява сигурно хеширане на пароли.
...
Мобилното приложение използва Expo \code{54}, React Native \code{0.81.5}, React \code{19.1.0} и TypeScript. Навигацията е реализирана с React Navigation, а сесиите се съхраняват чрез Expo Secure Store. Проектът използва \code{react-native-safe-area-context}, за да се адаптира към реални iPhone екрани с notch или Dynamic Island.
...
Използваният набор от данни е \code{jtrotman/formula-1-race-data} от Kaggle, който представлява релационен Formula 1 архив в Ergast стил. Основните таблици са \code{results.csv}, \code{races.csv}, \code{circuits.csv}, \code{qualifying.csv}, \code{status.csv}, \code{drivers.csv}, \code{constructors.csv} и \code{seasons.csv}.
...
Използван е \code{XGBClassifier} със \code{scikit-learn Pipeline} и \code{ColumnTransformer}. Категорийните признаци се обработват чрез попълване на липсващи стойности и one-hot кодиране, а числовите чрез попълване с медиана. Основните параметри на модела са:
...
Като бъдещо развитие могат да се добавят данни от състезания в реално време, метеорологични данни в реално време, индивидуални SHAP обяснения, отделен модел за очаквана финална позиция, push известия и продукционно разгръщане. Въпреки тези възможни разширения текущата версия е достатъчно завършена за академична демонстрация и защита.
...
Категорийните признаци се обработват с one-hot кодиране, а числовите с попълване чрез медиана. След това XGBoost моделът се пакетира в scikit-learn pipeline, който може да бъде запазен и използван за извод. Този подход пази логиката за предварителна обработка заедно с модела и намалява риска от разминаване между обучение и предоставяне на прогнози.
